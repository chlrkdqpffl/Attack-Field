...
Texture2D<float> DepthTex : register(t10);
Texture2D<float3> NoramlTex : register(t11);
...
cbuffer SSReflectionPSConstants : register(b4)
{
...
/*
// 임시로 아래는 사용 안하는중.. 반사 구현되면 아래로 변경하기
...
VS_OUTPUT SSReflectionVS(float4 pos : POSITION, float3 norm : NORMAL)
{
    ...
    Output.Position = mul(pos, gmtxView * gmtxProjection);
	// Transform the position and normal to view space
    Output.ViewPosition = mul(pos, gmtxView);
    Output.ViewNormal = mul(norm, (float3x3) gmtxView);
    */

    // Transform to projected space
    //output.position = mul(mul(mul(float4(input.position, 1.0f), gmtxWorld), gmtxView), gmtxProjection);
    //Output.Position = mul(pos, WorldViewProjection);
    Output.Position = mul(mul(pos, gmtxWorld), WorldViewProjection);
...
cbuffer SSReflectionPSConstants : register(b10)     // PS Set
{
...
//static const float PixelSize = 2.0 / 900.0f;
static const float PixelSize = 2.0 / g_f4Var.x;

// Number of sampling steps
// This is resulotion dependent
// Pick the maximum of the HDR width and height
//static const int nNumSteps = 1600;
static const int nNumSteps = g_f4Var.y;

float4 SSReflectionPS(VS_OUTPUT In) : SV_TARGET
{
//    return float4(0.7f, 0.0f, 0.0f, 0.0f);
