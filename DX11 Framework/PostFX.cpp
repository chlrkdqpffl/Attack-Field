...
CPostFX::~CPostFX()
{
	if (m_pTextScreenMesh)m_pTextScreenMesh->Release();
}
...
HRESULT CPostFX::Init(ID3D11Device *pd3dDevice, UINT width, UINT height)
{
	...
	DXUT_SetDebugName( m_pDownScale1DBuffer, "P_m_pDownScale1DBuffer" );
	...
	DXUT_SetDebugName( m_pDownScale1DUAV, "P_m_pDownScale1DUAV" );
	...
	DXUT_SetDebugName( m_pDownScale1DSRV, "P_m_pDownScale1DSRV" );
	...
	DXUT_SetDebugName(m_pSampLinear, "Linear");

	samDesc.Filter = D3D11_FILTER_MIN_MAG_MIP_POINT;
	V_RETURN(pd3dDevice->CreateSamplerState(&samDesc, &m_pSampPoint));
	DXUT_SetDebugName(m_pSampPoint, "Point");

	Read_CSO_Shader(pd3dDevice);

	m_pTextScreenMesh = new CTextureToScreenRectMesh(pd3dDevice);
...
void CPostFX::Read_CSO_Shader(ID3D11Device *pd3dDevice)
{
	...
	//#define _USE_CSO
		...
		DXUT_SetDebugName(m_pFullScreenQuadVS, "Post FX - Vertical Blur CS");
	...
	DXUT_SetDebugName(m_pDownScaleFirstPassCS, "m_pDownScaleFirstPassCS");
	...
	DXUT_SetDebugName(m_pFullScreenQuadVS, "m_pFullScreenQuadVS");

	//CShader::CreatePixelShaderFromCompiledFile(pd3dDevice, TEXT("PSPostProcessing.cso"), &m_pFinalPassPS);
	CShader::CreatePixelShaderFromFile(pd3dDevice, L"PostFX.hlsl", "FinalPassPS", "ps_5_0", &m_pFinalPassPS);
	DXUT_SetDebugName(m_pFinalPassPS, "m_pFinalPassPS");
...
void CPostFX::FinalPass(ID3D11DeviceContext* pd3dImmediateContext, ID3D11ShaderResourceView* pLDRSRV)
{
	...
	m_pTextScreenMesh->Render(pd3dImmediateContext);
